...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		int categ = pid.pgcateg();
		if(categ == BTreePageId.LEAF){
			return (BTreeLeafPage) this.getPage(tid,dirtypages,pid,perm);
		}else if ( categ == BTreePageId.INTERNAL || categ == BTreePageId.ROOT_PTR){
				BTreeInternalPage internal = (BTreeInternalPage) this.getPage(tid,dirtypages,pid,perm);

				Iterator<BTreeEntry> pageIditer = internal.iterator();
				if(!pageIditer.hasNext()){
					return null;
				}
				BTreeEntry staticEntry = pageIditer.next();
				if(f==null || staticEntry.getKey().compare(Op.GREATER_THAN_OR_EQ,f)) {
					return findLeafPage(tid,dirtypages,staticEntry.getLeftChild(),perm,f);
				}
				while(pageIditer.hasNext()){
					staticEntry = pageIditer.next();
					if(staticEntry.getKey().compare(Op.GREATER_THAN_OR_EQ,f)) {
						return findLeafPage(tid, dirtypages, staticEntry.getLeftChild(), perm, f);
					}
				}
				return findLeafPage(tid, dirtypages, staticEntry.getRightChild(), perm, f);
		} else {
			throw new DbException("unsupported categ(included header)");
		}
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        if(page.getRightSiblingId()!=null){
            BTreeLeafPage prevRightPage = (BTreeLeafPage)getPage(tid,dirtypages,page.getRightSiblingId(),Permissions.READ_WRITE);
            prevRightPage.setLeftSiblingId(rightPage.pid);
        }
        rightPage.setLeftSiblingId(page.pid);
        page.setRightSiblingId(rightPage.pid);
        // moving half of the tuples to the new page.
        Iterator<Tuple> tupleIter = page.reverseIterator();
        int usedSlotNum = page.getNumTuples();
        for(int i = 0; i < usedSlotNum/2; i++){
            if(!tupleIter.hasNext()){
                throw new DbException("the num of used slot is incorrect");
            } else{
        ...
        if(!tupleIter.hasNext()){
            throw new DbException("no more tuple");
        }
        Tuple midTuple = tupleIter.next();
        Field copyUpField = midTuple.getField(keyField);
        BTreeEntry CopyUpEntry = new BTreeEntry(copyUpField,page.pid,rightPage.pid);
        BTreeInternalPage parentPage = getParentWithEmptySlots(tid,dirtypages,page.getParentId(),copyUpField);
        parentPage.insertEntry(CopyUpEntry);
        updateParentPointers(tid,dirtypages,parentPage);
        if(copyUpField.compare(Op.GREATER_THAN_OR_EQ, field)){
            return page;
        }else{
            return rightPage;
        }
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        int usedSlotNum = page.getNumEntries();
        for(int i = 0; i < usedSlotNum/2; i++){
            if(!entryIter.hasNext()){
                throw new DbException("the num of used slot is incorrect");
            } else{
                BTreeEntry entry = entryIter.next();
                page.deleteKeyAndRightChild(entry);
                rightPage.insertEntry(entry);
            }
        }
        if(!entryIter.hasNext()){
            throw new DbException("no more entry");
        }
        BTreeEntry popUpEntry = entryIter.next();
        Field popUpField = popUpEntry.getKey();
        page.deleteKeyAndRightChild(popUpEntry);
        BTreeInternalPage parentPage = getParentWithEmptySlots(tid,dirtypages,page.getParentId(),popUpField);
        popUpEntry = new BTreeEntry(popUpField, page.pid, rightPage.pid);
        parentPage.insertEntry(popUpEntry);
        updateParentPointers(tid, dirtypages, parentPage);
        updateParentPointers(tid, dirtypages, page);
        updateParentPointers(tid, dirtypages, rightPage);
        if(popUpField.compare(Op.GREATER_THAN_OR_EQ, field)){
            return page;
        }else{
            return rightPage;
        }
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        if(sibling.getNumTuples()==0||page.getNumTuples()==page.getMaxTuples()){
            throw new DbException("There is no enough tuple to spare or the size of page is maxi");
        }
		int numSteal = (sibling.getNumTuples() - page.getNumTuples()) / 2;
        Iterator <Tuple> entryIter;
        if(isRightSibling){
            entryIter = sibling.iterator();
        }else{
            entryIter = sibling.reverseIterator();
        }
        Tuple moveTuple = null;
        for(int i=0 ; i< numSteal; i++){
            if(entryIter.hasNext()){
                moveTuple = entryIter.next();
                sibling.deleteTuple(moveTuple);
                page.insertTuple(moveTuple);
            }
        }
        entry.setKey(moveTuple.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        if(leftSibling.getNumEntries()==0){
            throw new DbException("The left sibiling is null");
        }
		int numSteal = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
        Iterator<BTreeEntry> leftEntryIter = leftSibling.reverseIterator();
		if(!page.iterator().hasNext()){
			throw new  DbException("iterator is not enough");
		}
        BTreePageId rightChild = page.iterator().next().getLeftChild();
        BTreeEntry addedEntry = new BTreeEntry(parentEntry.getKey(), null,page.iterator().next().getLeftChild());
        for (int i = 0; i < numSteal; i ++) {
            // The rightSibiling MUST small that the maxEntries
            if(!leftEntryIter.hasNext()){
                throw new DbException("no more left sibiling");
            }
            BTreeEntry leftEntryIterRight = leftEntryIter.next();
            addedEntry.setLeftChild(leftEntryIterRight.getRightChild());
            page.insertEntry(addedEntry);
            addedEntry = new BTreeEntry(leftEntryIterRight.getKey(), null, leftEntryIterRight.getRightChild());
			leftSibling.deleteKeyAndRightChild(leftEntryIterRight);
        }
        parentEntry.setKey(addedEntry.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        Iterator<BTreeEntry> rightEntryIter = rightSibling.iterator();
        if(!page.reverseIterator().hasNext()){
			throw new  DbException("iterator is not enough");
        }
        BTreeEntry addedEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), null);
        for (int i = 0; i <numSteal; i ++) {
            // The rightSibiling MUST small that the maxEntries
            if(!rightEntryIter.hasNext()){
                throw new DbException("no more left sibiling");
            }
            BTreeEntry rightEntryIterLeft = rightEntryIter.next();
            addedEntry.setRightChild(rightEntryIterLeft.getLeftChild());
            page.insertEntry(addedEntry);
            addedEntry = new BTreeEntry(rightEntryIterLeft.getKey(), rightEntryIterLeft.getLeftChild(), null);
			rightSibling.deleteKeyAndLeftChild(rightEntryIterLeft);
        }
        parentEntry.setKey(addedEntry.getKey());
        parent.updateEntry(parentEntry);
        updateParentPointers(tid,dirtypages,page);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        if(rightPage.getRightSiblingId()!=null){
            BTreeLeafPage newRightPage = (BTreeLeafPage)getPage(tid,dirtypages, rightPage.getRightSiblingId(),Permissions.READ_WRITE);
            newRightPage.setLeftSiblingId(leftPage.pid);
        }
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        Iterator<BTreeEntry> rightEntryIter = rightPage.iterator();
        if(!rightEntryIter.hasNext()){
            throw new DbException("do not have entry on the right page");
        }
        BTreeEntry firstRightEntry = rightEntryIter.next();
        Iterator<BTreeEntry> leftEntryIter = leftPage.reverseIterator();
        if(!leftEntryIter.hasNext()){
            throw new DbException("do not have entry on the left page");
        }
        BTreeEntry lastLeftEntry = leftEntryIter.next();
        BTreeEntry firstInsertedEntry = new BTreeEntry(parentEntry.getKey(),lastLeftEntry.getRightChild(),
                firstRightEntry.getLeftChild());
        leftPage.insertEntry(firstInsertedEntry);
        rightEntryIter = rightPage.iterator();
        while(rightEntryIter.hasNext()){
            BTreeEntry entry = rightEntryIter.next();
